...
def get_presigned_urls(bucket_name, key_name) -> None:
    ...
    for method in client_method_dict.keys():
        url = s3.generate_presigned_url(
            ClientMethod=client_method_dict[method], Params={"Bucket": bucket_name, "Key": key_name}
        )
...
def main():
    ...
    parser.add_argument(
        "--bucket", action="store", required=True, dest="bucket_name", help="The name of the S3 bucket of interest."
    )
    parser.add_argument(
        "--key",
        action="store",
