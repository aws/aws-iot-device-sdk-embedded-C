...
static void aws_iot_mqtt_tests_message_aggregator( AWS_IoT_Client * pClient,
                                                   char * topicName,
...
static void aws_iot_mqtt_tests_disconnect_callback_handler( AWS_IoT_Client * pClient,
                                                            void * param )
{
    IOT_UNUSED( pClient );
    IOT_UNUSED( param );
...
static IoT_Error_t aws_iot_mqtt_tests_subscribe_to_test_topic( AWS_IoT_Client * pClient,
                                                               QoS qos,
...
int aws_iot_mqtt_tests_multi_threading_validation()
{
    ...
    do
    {
        ...
        for( i = 0; i < MAX_PUB_THREAD_COUNT; i++ )
        {
            finishedThreadCount += threadStatus[ i ];
        }
