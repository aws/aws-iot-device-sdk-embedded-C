...
void InitMQTTParamsSetup( IoT_Client_Init_Params * params,
                          char * pHost,
                          uint16_t port,
...
void ConnectMQTTParamsSetup( IoT_Client_Connect_Params * params,
                             char * pClientID,
...
void ConnectMQTTParamsSetup_Detailed( IoT_Client_Connect_Params * params,
                                      char * pClientID,
                                      uint16_t clientIDLen,
                                      QoS qos,
                                      bool isCleanSession,
...
void setTLSRxBufferForConnack( IoT_Client_Connect_Params * params,
...
void setTLSRxBufferForConnackAndSuback( IoT_Client_Connect_Params * conParams,
                                        unsigned char sessionPresent,
                                        char * topicName,
                                        size_t topicNameLen,
                                        QoS qos )
...
void setTLSRxBufferForDoubleSuback( char * topicName,
                                    size_t topicNameLen,
                                    QoS qos,
...
void setTLSRxBufferForSuback( char * topicName,
                              size_t topicNameLen,
                              QoS qos,
                              IoT_Publish_Message_Params params )
{
...
void setTLSRxBufferDelay( int seconds,
                          int microseconds )
{
...
void setTLSRxBufferWithMsgOnSubscribedTopic( char * topicName,
                                             size_t topicNameLen,
                                             QoS qos,
                                             IoT_Publish_Message_Params params,
                                             char * pMsg )
...
unsigned char generateMultipleSubTopics( char * des,
                                         int boundary )
{
    int i;
    int currLen = 0;
...
void encodeRemainingLength( unsigned char * buf,
                            size_t * st,
                            size_t length )
{
    unsigned char c;
    ...
    do
    {
        ...
        if( length > 0 )
        {
            c |= ( unsigned char ) ( 0x80 );        /* If there is still another byte following */
        }
...
unsigned char * connectTxBufferHeaderParser( ConnectBufferProofread * params,
                                             unsigned char * buf )
{
...
bool isConnectTxBufFlagCorrect( IoT_Client_Connect_Params * settings,
                                ConnectBufferProofread * readRes )
{
    bool ret = true;
    int i;
...
bool isConnectTxBufPayloadCorrect( IoT_Client_Connect_Params * settings,
...
const char * getJobTopicTypeName( AwsIotJobExecutionTopicType topicType )
{
    switch( topicType )
    {
        case JOB_UPDATE_TOPIC:
            return "JOB_UPDATE_TOPIC";

        case JOB_NOTIFY_TOPIC:
            return "JOB_NOTIFY_TOPIC";

        case JOB_NOTIFY_NEXT_TOPIC:
            return "JOB_NOTIFY_NEXT_TOPIC";

        case JOB_GET_PENDING_TOPIC:
            return "JOB_GET_PENDING_TOPIC";

        case JOB_DESCRIBE_TOPIC:
            return "JOB_DESCRIBE_TOPIC";

        case JOB_START_NEXT_TOPIC:
            return "JOB_START_NEXT_TOPIC";

        case JOB_WILDCARD_TOPIC:
            return "JOB_WILDCARD_TOPIC";

        case JOB_UNRECOGNIZED_TOPIC:
            return "JOB_UNRECOGNIZED_TOPIC";
...
const char * getJobReplyTypeName( AwsIotJobExecutionTopicReplyType replyType )
{
    switch( replyType )
    {
        case JOB_REQUEST_TYPE:
            return "JOB_REQUEST_TYPE";

        case JOB_ACCEPTED_REPLY_TYPE:
            return "JOB_ACCEPTED_REPLY_TYPE";

        case JOB_REJECTED_REPLY_TYPE:
            return "JOB_REJECTED_REPLY_TYPE";

        case JOB_WILDCARD_REPLY_TYPE:
            return "JOB_WILDCARD_REPLY_TYPE";

        case JOB_UNRECOGNIZED_TOPIC_TYPE:
            return "JOB_UNRECOGNIZED_TOPIC_TYPE";
